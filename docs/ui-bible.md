...
- **Monster editor — creature sheet (Monsters tab):** do **not** render a **Description** subsection above the stat block. The normalized `description` field on monster entries is usually compendium boilerplate (size, origin, general type text), not distinct per-creature information; showing it adds noise without helping run the monster. Use **Tactics**, the stat block, and **Sources** for DM-facing content. **Exception:** template and create-template flows may still show **Description** when editing PDF-derived template or import text—that content is intentional overlay prose, not the same field semantics as generated monster entries.
