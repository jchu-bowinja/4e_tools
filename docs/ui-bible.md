...
- **Monster editor — creature sheet (Monsters tab):** one column shell (`sheetPanel`) per center column; **Tactics** and **Sources** use flow spacing only (`centerFlowSubsectionStyle`)—no nested bordered panel inside the column. The stat block card keeps its own border. Do **not** render a **Description** subsection above the stat block: the normalized `description` field is usually compendium boilerplate (size, origin, general type text), not distinct per-creature information. Use **Tactics**, the stat block, and **Sources** for DM-facing content. **Exception:** template and create-template flows may still show **Description** when editing PDF-derived template or import text.
